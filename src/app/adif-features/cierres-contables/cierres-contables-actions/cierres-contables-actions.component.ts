...
import { takeUntil, distinctUntilChanged, throttleTime, switchMap } from 'rxjs/internal/operators';
...
export class CierresContablesActionsComponent implements OnInit, OnDestroy {
  ...
  listeningPortService() {
    ...
    .pipe(
        ...
        throttleTime(3000),
        switchMap(() => this.cierresContablesServiceService.podListner())
    )
    .subscribe(data => {
      if (data) {
